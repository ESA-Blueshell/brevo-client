...
- [ ] `specs/brevo.json` was changed only by `tools/src/cli.mjs sync`, never by hand
- [ ] `typescript/src/generated` was regenerated if the spec moved (`npm run generate`)
- [ ] Both clients build and their tests pass
- [ ] If a tag was added or removed, `specs/surface.json` reflects it and the surface test's operation list was updated
