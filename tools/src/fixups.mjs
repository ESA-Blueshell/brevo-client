 ...
 *
 * These are deliberately NOT baked into `specs/brevo.json`. That file is the
 * versioned contract and is compared against upstream to decide release numbers,
 * so it stays faithful to what upstream actually publishes. Rewriting a shape
 * in the vendored copy would make the committed spec lie and would hide the
 * rewrite from the diff a reviewer reads. Confining the hacks here keeps the
 * contract honest and the workarounds visible.
 *
 * `dropConstraintOnlyCompositions` is the one Brevo needs. The other two are
 * carried unchanged from the Discord client so both repositories apply an
 * identical set; they are harmless no-ops against Brevo's document.
 */
