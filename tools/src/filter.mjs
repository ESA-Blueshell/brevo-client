 ...
 *
 * Why this exists at all: Brevo publishes ~380 operations across 36 tags in a
 * 1.15 MB document, and this client binds two tags. Generating the whole
 * surface would be slow, would expose endpoints nobody intends to call, and —
 * the reason that actually forces the issue — would make versioning
 * meaningless: any change anywhere in Brevo's document would move this
 * client's version, so the number would describe upstream's release cadence
 * rather than this client's compatibility.
 *
 * So the filtered surface, not the upstream document, is the versioned
 * contract. Filtering takes the 1.15 MB document to 320 KB and 52 operations.
 */
