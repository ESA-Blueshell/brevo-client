 ...
 * identical set, and so `specs/brevo.json` stays faithful to what upstream
 ...
 */
val specSurface = layout.projectDirectory.file("../specs/brevo.json")
    ...
    doFirst {
        ...
        require(probe == "yes") {
            "`node` was not found on PATH. The generated client is derived from " +
                "../specs/brevo.json by tools/src/cli.mjs, which needs Node 22+ " +
...
val generateClient = tasks.register<GenerateTask>("generateClient") {
    ...
    // the repo, and openapi-generator's own validator objects to constructs
    // Brevo legitimately uses.
    ...
    packageName.set("net.blueshell.clients.brevo")
    ...
    // Brevo declares only 15 named schemas and inlines the rest, so
    // openapi-generator synthesises names for inline shapes. A few of those
    // synthesised models are unusable — free-form maps and polymorphic
    // identifiers it renders as broken types — and are mapped to plain
    // Kotlin/Java types instead. Carried over from ESA-Blueshell/website,
    // where each entry was added in response to a specific compile failure.
    schemaMappings.set(
        mapOf(
            ...
            "postContactInfo_contacts_success" to "kotlin.Any",
            "postContactInfo_contacts_failure" to "kotlin.Any",
        ),
    )

    // openapi-generator normalises Brevo's raw tags into class names, so these
    // are "TransactionalEmails" and "Contacts" while specs/surface.json names
    // the raw tags "Transactional emails" and "Contacts". Keep the two in step.
...
publishing {
    publications {
        create<MavenPublication>("maven") {
            ...
            artifactId = "brevo-client"
            pom {
                name.set("Brevo API client (Kotlin)")
                description.set(
                    "Kotlin Spring RestClient for the subset of the Brevo API " +
                        "declared in specs/surface.json. Generated; do not edit by hand.",
                )
                url.set("https://github.com/ESA-Blueshell/brevo-client")
                licenses {
                    license {
                        name.set("MIT")
                        url.set("https://github.com/ESA-Blueshell/brevo-client/blob/main/LICENSE")
                    }
                }
                scm {
                    url.set("https://github.com/ESA-Blueshell/brevo-client")
                    connection.set("scm:git:https://github.com/ESA-Blueshell/brevo-client.git")
                }
            }
        }
    }
